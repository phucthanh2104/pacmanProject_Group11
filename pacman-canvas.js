...
function geronimo() {
	...
	function Game() {
		...
		// Khởi tạo hàm newGame
		// Chức năng này cũng có thể được sử dụng để gọi lại khi nhấn nút restart
		// hàm này dùng được cho cả 2 chức năng New game và restart
		// arlet để xuất hiện ra 1 bảng "are you sure you want to restart?"
		// khi nhấn "OK" thì sự kiện này sẽ được thực thi
		// dùng console.log để test xem đã bắt được sự kiện chưa
		// set init(0)
		// sau đó gọi lại hàm forceResum để thực hiện tạo ra game mới
	...
	$(document).ready(function () {
		...
		// đây là đoạn code sử dụng jQuery để lắng nghe sự kiện click trên các phần tử có class "button" và id "instructions".
		// Sử dụng jQuery selector: $(document).on('click', '.button#instructions',
		// function (event) sử dụng cú pháp jQuery để chọn các phần tử có class "button" và id "instructions" trên document.
		// Gọi phương thức jQuery: game.showContent(); sử dụng cú pháp jQuery để gọi phương thức showContent() của một đối tượng có tên game
		// Trong đây 1 bộ phận thẻ div có id là #instructions đã được tạo sẵn
		// Khi click chuột và nút Instructions thì hàm này sẽ được gọi, lúc đó div này sẽ được hiển thị ra
		// 
		// 
		...
		//
		// đây là đoạn code sử dụng jQuery để lắng nghe sự kiện click trên các phần tử có class "button" và id "restart". 
		// Sử dụng jQuery selector: $(document).on('click', '.button#restart',
		// function (event) sử dụng cú pháp jQuery để chọn các phần tử có class "button" và id "restart" trên document.
		// Gọi phương thức jQuery: game.newGame(); sử dụng cú pháp jQuery để gọi phương thức newGame() của một đối tượng có tên game
		// Thư viện jquery đã được bổ sung ở đầu trang
		$(document).on('click', '.button#restart', function (event) {
			game.newGame();
		});
		// back button
